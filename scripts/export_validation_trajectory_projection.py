#!/usr/bin/env python3
"""Export 2D validation trajectory projections from fused embeddings."""
        ...
        run_payloads.append(
            {
                ...
                "baseline_fused_embeddings": np.load(
                    output_dir_path / "baseline_fused_embeddings.npy"
                ),
                "perturbed_fused_embeddings": np.load(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export validation trajectory projections")
    parser.add_argument(
        "--validation-manifest",
        type=str,
        required=True,
    ...
    output_path = (
        ...
        else validation_manifest_path.parent / "validation_trajectory_projection.json"
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("w", encoding="utf-8") as handle:
        json.dump(projection, handle, indent=2)

    logger.info("Saved validation trajectory projection to %s", output_path)
